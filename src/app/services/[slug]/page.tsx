...
const serviceData: Record<string, { title: string; desc: string; paragraphs: string[]; relatedServices: string[] }> = {
  "new-installations": {
    ...
    <div className="page-enter">
      ...
      <section className="pt-32 pb-8">
        <div className="section-wrap">
          <Link
            href="/services"
            className="inline-flex items-center gap-1.5 text-sm text-white/40 hover:text-[var(--color-orange)] transition-colors mb-8"
          >
            <ArrowLeft size={14} /> All Services
          </Link>

          <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[var(--color-orange)] mb-4">
            Service Detail
          </p>
          <h1 className="text-4xl md:text-5xl font-bold tracking-tight mb-3 max-w-2xl leading-[1.1]">
            {service.title}
          </h1>
          <div className="w-16 h-1 rounded-full bg-gradient-to-r from-[var(--color-orange)] to-[var(--color-orange-hover)] mt-6" />
        </div>
      </section>
      ...
      <section className="py-14 border-t border-white/[0.04]">
        <div className="section-wrap">
          <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[var(--color-orange)] mb-5">
            Related Services
          </p>
          <div className="flex flex-wrap gap-3">
            {service.relatedServices.map((rs) => (
              <Link
                key={rs}
                href={`/services/${rs}`}
                className="px-5 py-3 rounded-xl bg-white/[0.03] border border-white/[0.06] text-sm text-white/50 hover:text-white hover:bg-[var(--color-orange)]/10 hover:border-[var(--color-orange)]/20 transition-all"
              >
                {slugToTitle[rs] || rs}
              </Link>
            ))}
